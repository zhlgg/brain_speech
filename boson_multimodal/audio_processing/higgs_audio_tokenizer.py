...
class HiggsAudioTokenizer(nn.Module):
    ...
    def encode(self, audio_path_or_wv, sr=None, loudness_normalize=False, loudness_threshold=-23.0):
        if isinstance(audio_path_or_wv, str):
            if audio_path_or_wv[-3:] == 'wav':
                wv, sr = librosa.load(audio_path_or_wv, mono=True, sr=None)
            elif audio_path_or_wv[-4:] == 'flac' or audio_path_or_wv[-3:] == 'mp3':
                wv, sr = torchaudio.load(audio_path_or_wv)
                if wv.shape[0] > 1: wv = wv.mean(dim=0, keepdim=True)
                if sr != self.sample_rate: wv = torchaudio.functional.resample(wv, orig_freq=sr, new_freq=self.sample_rate)
            else:
                print('type error!')
                return None
        else:
