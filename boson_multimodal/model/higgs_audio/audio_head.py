...
class HiggsAudioDecoderProjector(HiggsAudioPreTrainedModel):
    ...
    def forward(
        ...
        with nvtx.range("decode_proj: total", color="blue"):  # 总范围标记
            # 1. 标记 if 条件判断（audio_out_mask.any() 的耗时）
            with nvtx.range("wzb check audio_out_mask.any()", color="green"):
                has_audio = audio_out_mask.any()

            if has_audio:
                # 标记 audio_lm_head 前向计算
                with nvtx.range("wzb audio_lm_head forward", color="red"):
                    audio_logits = self.audio_lm_head(hidden_states[audio_out_mask])
                
                # # 4. 标记设备同步操作（to(device)）
                # with nvtx.range("wzb sync audio_logits to device", color="purple"):
                #     audio_logits = audio_logits.to(hidden_states.device)
            
            else:
                # 5. 标记 else 分支的空 tensor 创建
                with nvtx.range("wzb create empty audio_logits", color="cyan"):
                    # audio_logits = torch.empty(
                    #     (0, self.config.audio_num_codebooks * (self.config.audio_codebook_size + 2)),
                    #     device=hidden_states.device,
                    #     dtype=hidden_states.dtype
