...
class HiggsAudioSampleCollator:
    ...
    def __call__(self, batch: List[ChatMLDatasetSample]):
        ...
        for i in range(len(processed_batch)):
            audio_in_mask = processed_batch[i].input_ids == self.audio_in_token_id #128015
            audio_out_mask = processed_batch[i].input_ids == self.audio_out_token_id  #128016
            audio_ids = torch.ones_like(processed_batch[i].input_ids) ## text len
            ...
            if self.encode_whisper_embed:
                for audio_in_ids_idx, idx in enumerate(audio_in_ids):
                    wv, sr = processed_batch[i].get_wv(audio_in_ids_idx)
                    resampled_wv = wv.cpu().numpy()
