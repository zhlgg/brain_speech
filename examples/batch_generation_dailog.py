...
class HiggsAudioBatchModelClient:
    ...
    def generate_single(
        ...
        ## split two speaker
        wv_1 = self._audio_tokenizer.decode(audio_out_ids_l[0].unsqueeze(0))[0, 0]
        wv_2 = self._audio_tokenizer.decode(audio_out_ids_l[1].unsqueeze(0))[0, 0]
        ...
        return wv_1, wv_2, sr, text_result
    ...
    def generate_batch(self, batch_data_list, **generation_kwargs):
        ...
        for i, data in enumerate(tqdm.tqdm(batch_data_list, desc="Processing batch")):
            ...
            try:
                waveform1, waveform2, sr, text_output = self.generate_single(**data, **generation_kwargs)
                results.append({
                    'waveform1': waveform1,
                    'waveform2': waveform2,
                    'sample_rate': sr,
...
def main(
    ...
    for batch_start in range(0, len(text_files), batch_size):
        ...
        for raw_json in batch_files:
            ...
            a_dir = os.path.join(output_dir, 'answers')
            os.makedirs(q_dir, exist_ok=True)
            os.makedirs(a_dir, exist_ok=True)
            output_path1 = os.path.join(a_dir, f"{base_name}.wav")
            output_path2 = os.path.join(q_dir, f"{base_name}.wav")

            batch_output_paths1.append(output_path1)
            batch_output_paths2.append(output_path2)
        ...
        if accelerator.is_main_process:
            for i, (result, output_path1, output_path2) in enumerate(zip(results, batch_output_paths1, batch_output_paths2)):
                if result['success']:
                    sf.write(output_path1, result['waveform1'], result['sample_rate'])
                    sf.write(output_path2, result['waveform2'], result['sample_rate'])
                    logger.info(f"Saved audio: {output_path1}")
                else:
